...
function acquireLock() {
    const lockFile = path.join(process.cwd(), 'a-plan.pid');
    ...
    try {
        if (fs.existsSync(lockFile)) {
            ...
            if (oldPid && !isNaN(oldPid)) {
                try {
                    ...
                    logger.error(`[Master] 检测到已有实例在运行 (PID: ${oldPid})`);
                    logger.error(`[Master] 请先停止旧实例: kill ${oldPid}`);
                    logger.error(`[Master] 或使用: pkill -f "master.js"`);
                    process.exit(1);
                } catch (e) {
                    logger.info(`[Master] 发现旧的 PID 文件 (${oldPid})，进程已不存在，清理并继续`);
                }
            }
        }
        
        fs.writeFileSync(lockFile, process.pid.toString());
        logger.info(`[Master] 进程锁已创建: ${lockFile}`);
        ...
        logger.warn(`[Master] 进程锁检查失败: ${error.message}，继续启动`);
    }
}
