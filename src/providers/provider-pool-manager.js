...
export class ProviderPoolManager {
    ...
    _buildHealthCheckRequests(providerType, modelName) {
        ...
        if (providerType && providerType.startsWith('gemini')) {
            requests.push({
                contents: [{
                    role: 'user',
        ...
        if (providerType && providerType.startsWith('claude-kiro')) {
            requests.push({
    ...
    _getBaseProviderType(providerType) {
        ...
        for (const key of Object.keys(ProviderPoolManager.DEFAULT_HEALTH_CHECK_MODELS)) {
            if (providerType && (providerType === key || providerType.startsWith(key + '-'))) {
                return key;
            }
        }
