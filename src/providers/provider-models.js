...
export function customModelMatchesProvider(modelConfig, providerType) {
    ...
    return listProvider === providerType || (listProvider && providerType && providerType.startsWith(listProvider + '-'));
}
...
export function extractModelIdsFromNativeList(modelList, providerType) {
    ...
    if (providerType && providerType !== MODEL_PROVIDER.OPENAI_CUSTOM && !providerType.startsWith(MODEL_PROVIDER.OPENAI_CUSTOM + '-')) {
        try {
...
export function getConfiguredSupportedModels(providerType, providerConfig = {}) {
    ...
    // 对于 managed provider，也支持从其他格式获取
...
export function getProviderModels(providerType) {
    ...
    if (PROVIDER_MODELS[providerType]) {
        ...
        for (const key of Object.keys(PROVIDER_MODELS)) {
            if (providerType && providerType.startsWith(key + '-')) {
        ...
        // 对 DeepSeek 变体的特殊支持
        if (models.length === 0 && /^(deepseek|ds)[-_]/i.test(providerType)) {
            models = [...PROVIDER_MODELS['deepseek']];
        }
        
        // 对 OpenAI 兼容 API 的通用支持
        if (models.length === 0 && /^(openai|oai)[-_]?/i.test(providerType)) {
            models = [...PROVIDER_MODELS['openai-custom']];
        }
    }
