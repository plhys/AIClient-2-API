...
**解决方案**:
1. 使用 `openai-custom` 提供商类型并配置 Cloudflare Gateway 的 URL
2. 或者使用显式前缀：`openai-custom:@cf/meta/llama-3.1-8b-instruct`
3. 配置自定义模型别名

**注**: 原 `cloudflare-gateway-free` 提供商类型已移除，统一使用 `openai-custom` 透明代理
...
**注**: v5.1.0 起，原 `cloudflare-gateway-free` 已统一为 `openai-custom` 提供商类型
