...
async function insertComponent(componentPath, container, position = 'beforeend') {
    ...
    // 提取并移除脚本
    const scripts = [];
    let processedHtml = html.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, (match, code) => {
        scripts.push(code);
        return '';
    });
    ...
    if (position === 'replace') {
        ...
        containerElement.insertAdjacentHTML(position, processedHtml);
    }
...
async function initializeComponents() {
    ...
    try {
        ...
        const sectionComponents = [
            ...
            { path: `${basePath}section-logs.html`, container: '#content-container', position: 'beforeend' },
            { path: `${basePath}section-plugins.html`, container: '#content-container', position: 'beforeend' },
            { path: `${basePath}section-terminal.html`, container: '#content-container', position: 'beforeend' },
            { path: `${basePath}section-network.html`, container: '#content-container', position: 'beforeend' },
        ];
