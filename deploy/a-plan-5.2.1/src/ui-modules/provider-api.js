...
async function getManagedSupportedModels(providerType, providers = [], currentConfig = {}) {
    const allModels = [];
    
    for (const provider of providers) {
        // 1. 先加配置的 supportedModels
        const configured = getConfiguredSupportedModels(providerType, provider);
        if (configured.length > 0) {
            allModels.push(...configured);
            continue; // 有配置就不用远程获取了
        }
        
        // 2. 尝试远程获取（简单方式，避免卡住）
        try {
            // 直接发个简单请求测试连通性
            const baseUrl = provider.OPENAI_BASE_URL || provider.baseUrl;
            if (!baseUrl) continue;
            ...
            const response = await fetch(testUrl, {
                method: 'GET',
                headers: { 'Authorization': `Bearer ${provider.OPENAI_API_KEY || provider.apiKey || ''}` },
            ...
            if (response && response.ok) {
                ...
                if (data.data && Array.isArray(data.data)) {
                    allModels.push(...data.data.map(m => m.id));
                }
            }
        } catch (e) {
            // 忽略错误
    ...
    return normalizeModelIds(allModels);
}
...
export async function handleGetProviderModels(req, res, currentConfig, providerPoolManager) {
    ...
    for (const type of allTypes) {
        ...
        // 合并预定义模型和用户配置的 supportedModels
        const providers = providerPools[type] || [];
        if (providers.length > 0) {
            const configuredModels = providers.flatMap(p => getConfiguredSupportedModels(type, p));
            if (configuredModels.length > 0) {
                // 合并并去重
                models = [...new Set([...models, ...configuredModels])];
            }
        }
        
        // 对于 managed provider，尝试从远程 API 获取模型列表
        if (usesManagedModelList(type)) {
            const managedModels = await getManagedSupportedModels(type, providers, currentConfig);
            if (managedModels.length > 0) {
                models = [...new Set([...models, ...managedModels])];
            }
        }
...
export async function handleGetProviderTypeModels(req, res, currentConfig, providerPoolManager, providerType) {
    ...
    if (usesManagedModelList(providerType)) {
        try {
            ...
            const managedModels = await getManagedSupportedModels(providerType, providerPools[providerType] || [], currentConfig);
            if (managedModels.length > 0) {
