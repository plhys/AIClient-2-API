...
async function startServer() {
    ...
    serverInstance.listen(PORT, HOST, () => {
        const VERSION = process.env.APP_VERSION || '5.2.0';
        logger.info(`🚀 [A-Plan v${VERSION}] Listening on port ${PORT}. Hydrating core...`);
        
        // 启动WebSSH服务（可选，带重试机制）
        let websshRetries = 0;
        const maxWebsshRetries = 3;
        async function startWebSSH() {
            try {
                const { WebSSHServer } = await import('../modules/ssh/webssh.js');
                const sshServer = new WebSSHServer(serverInstance, {
                    etSocksPort: parseInt(process.env.ET_SOCKS_PORT) || 7890
                });
                sshServer.start();
                global.WEBSSH_READY = true;
                logger.info('[WebSSH] 网络终端服务已启动');
            } catch (err) {
                websshRetries++;
                if (websshRetries < maxWebsshRetries) {
                    logger.warn(`[WebSSH] 启动失败，${5000*websshRetries}ms后重试 (${websshRetries}/${maxWebsshRetries}):`, err.message);
                    setTimeout(startWebSSH, 5000 * websshRetries);
                } else {
                    logger.error('[WebSSH] 重试次数耗尽，放弃启动:', err.message);
                }
            }
        }
        startWebSSH();
...
async function bootstrapCore() {
    ...
    // 自动启动 EasyTier（带重试机制）
    let easytierRetries = 0;
    const maxEasytierRetries = 3;
    async function startEasyTier() {
        try {
            const { autoStart } = await import('../modules/network/easytier-manager.js');
            await autoStart();
            global.EASYTIER_READY = true;
        } catch (err) {
            easytierRetries++;
            if (easytierRetries < maxEasytierRetries) {
                logger.warn(`[EasyTier] 启动失败，${5000*easytierRetries}ms后重试 (${easytierRetries}/${maxEasytierRetries}):`, err.message);
                setTimeout(startEasyTier, 5000 * easytierRetries);
            } else {
                logger.error('[EasyTier] 重试次数耗尽，放弃自动启动:', err.message);
            }
        }
    }
    // 延迟5秒启动，确保其他服务就绪
    setTimeout(startEasyTier, 5000);
