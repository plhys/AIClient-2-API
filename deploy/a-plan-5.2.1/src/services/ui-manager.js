...
export async function handleUIApiRequests(method, pathParam, req, res, currentConfig, providerPoolManager) {
    ...
    // EasyTier 网络 API - 公开访问（不需要token验证）
    if (pathParam.startsWith('/api/network/')) {
        // 动态导入 EasyTier 管理器
        const { getStatus, startEasyTier, stopEasyTier, loadETConfig, saveETConfig, autoStart } = await import('../modules/network/easytier-manager.js');
        
        // 获取状态
        if (pathParam === '/api/network/status' && method === 'GET') {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: true, ...getStatus() }));
            return true;
        }
        
        // 获取配置
        if (pathParam === '/api/network/config' && method === 'GET') {
            const config = loadETConfig() || {
                vip: '',  // 留空，让用户填写
                networkName: '',
                networkSecret: '',
                relay: '',  // 留空，用户填写或留空使用默认对等节点发现
                noTun: true,
                latencyFirst: true,
                multiThread: true,
                autoStart: true
            ...
            if (safeConfig.networkSecret) safeConfig.networkSecret = '********';
            res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        // 启动 EasyTier
        if (pathParam === '/api/network/start' && method === 'POST') {
            ...
            try {
                await startEasyTier(body);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, message: 'EasyTier 已启动' }));
            } catch (err) {
        ...
        // 停止 EasyTier
        if (pathParam === '/api/network/stop' && method === 'POST') {
        ...
        // 保存配置
        if (pathParam === '/api/network/config' && method === 'POST') {
            const body = await new Promise(r => { 
                let b=''; 
                req.on('data', c=>b+=c); 
                req.on('end', () => {
    ...
    // Handle UI management API requests (需要token验证，除了公开接口)
    if (pathParam.startsWith('/api/') && 
        pathParam !== '/api/login' && 
        pathParam !== '/api/health' && 
        pathParam !== '/api/events' && 
        pathParam !== '/api/provider-models' &&
        !pathParam.startsWith('/api/provider-models/')) {
        // 检查token验证
    ...
    if (pathParam.startsWith('/api/network/')) {
        // 动态导入 EasyTier 管理器
        const { getStatus, startEasyTier, stopEasyTier, loadETConfig, saveETConfig } = await import('../modules/network/easytier-manager.js');
